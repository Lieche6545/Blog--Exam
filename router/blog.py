...
from schema.articles import ShowBlog, BlogCreate
...
async def list_blog_from_author(user_id, db:Session = Depends(default_Codes.get_db)):
...
async def create_article(article:BlogCreate, db:Session = Depends(default_Codes.get_db), token:str=Depends(oauth2_scheme)):
    
    user = default_Codes.get_user_from_token(db, token)
...
async def Edit_article(id:int, input:BlogCreate, db:Session = Depends(default_Codes.get_db), token:str=Depends(oauth2_scheme)):
...
async def delete_article(id:int, db:Session = Depends(default_Codes.get_db), token:str=Depends(oauth2_scheme)):
