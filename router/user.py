...
from schema.user import UserCreate, EditUser, ShowUser
...
async def register_user(new_user: UserCreate, db:Session=Depends(default_Codes.get_db)):
...
async def Edit_User_Info(id:int, input:EditUser, db:Session = Depends(default_Codes.get_db), token:str=Depends(oauth2_scheme)):
...
async def Delete_My_account(id:int, db:Session=Depends(default_Codes.get_db), token:str=Depends(oauth2_scheme)):
