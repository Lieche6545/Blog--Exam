...
def get_all_users(db:Session=Depends(default_Codes.get_db)):
...
def delete_user(id:int, db:Session = Depends(default_Codes.get_db)):
...
def delete_all_users(db:Session=Depends(default_Codes.get_db)):
...
# async def delete_article(id:int, db:Session = Depends(default_Codes.get_db)):
...
def delete_all_articles(db:Session=Depends(default_Codes.get_db)):
    all_blogs = db.query(Blogs).all()
